...
$Response = Invoke-WebRequest -Uri $AllURLs
$Script:RundeckDownloadURL = ($Response.Links | where-object {$_.innerText -like "*rundeckpro-ent*war*"}).href
...
Function Show-MainForm
{
    ...
    $Button3.add_Click({
        ...
        $BackgroundScript = [Powershell]::Create().AddScript({
            ...
            if (Select-String -InputObject $JavaVersionOut[0] -Pattern "java version")
            {
...
java %RDECK_CLI_OPTS% %RDECK_SSL_OPTS% -jar ' + $RundeckFleName + ' --skipinstall -d  >> %CURDIR%\var\logs\service.log  2>&1'
    ...
    $Button8.add_Click({
        ...
        $BackgroundScript = [Powershell]::Create().AddScript({
            ...
            $ExtMySQL = "rundeck.projectsStorageType=db",
                            ...
                            "dataSource.url = jdbc:mysql://$($Context.SQLServer)/rundeck?autoReconnect=true&useSSL=false&allowPublicKeyRetrieval=true",
                            "dataSource.username = $($Context.Username)",
                            "dataSource.password = $($Context.Password)"
    ...
    $Button10.add_Click({
        ...
        if ($SelectedUsersOU)
        {
            $TextBox11.Text = $SelectedUsersOU.DistinguishedName
        }

        $SelectedGroupsOU = $BgJobResult.OUs | Out-GridView -PassThru -Title "Select Groups OU"
        if ($SelectedGroupsOU)
        {
            $TextBox12.Text = $SelectedGroupsOU.DistinguishedName
        }
    })
    ...
    $Button11.add_Click({
        ...
        $BackgroundScript = [Powershell]::Create().AddScript({
            ...
            try {
                $LdapUsers = Get-ADObject -LDAPFilter "(objectClass=user)" -SearchBase $Context.OuSearch -Credential $AdCreds -Server $Context.Server
            
                $LdapGroups = Get-ADObject -LDAPFilter "(objectClass=group)" -SearchBase $Context.OuGroups -Credential $AdCreds -Server $Context.Server
    ...
    $Button12.add_Click({
        ...
        $UserDN = Get-ADUser -Filter ('SamAccountName -eq "' + $SamAccName + '"') | Select-Object -ExpandProperty DistinguishedName
    ...
    $Form.add_Load({
        ...
        {
            $MessageIcon = [System.Windows.MessageBoxImage]::Warning
            [System.Windows.MessageBox]::Show("Required modules missing: $MissingModules. Install and restart script",$MessageIcon)
        }
    })
